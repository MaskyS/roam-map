...
import { createMapMountLifecycle, identifyMapMount } from "../../src/ui/mount-maps.js";
...
test("mounts verified map buttons independently and restores them on cleanup", async () => {
  ...
  const mountIds = [...dom.window.document.querySelectorAll(".rrm-mount")].map((element) =>
    element.getAttribute("data-roam-map-mount-id"),
  );
  assert.notEqual(mountIds[0], mountIds[1]);
...
test("a synchronous root failure restores Roam's fallback control", async () => {
  const dom = new JSDOM(`
    <body><div data-block-uid="map"><button class="rm-xparser-default-map">map</button></div></body>
  `);
  const lifecycle = createMapMountLifecycle({
    document: dom.window.document,
    MutationObserver: dom.window.MutationObserver,
    ...
    api: { pull: async () => ({ ":block/string": "{{map}}" }) },
    createView: (identity) => identity,
  });
